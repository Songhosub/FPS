...
public class Assault : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Back = BackT.position;
            transform.position = Back + transform.forward * 0.5f + transform.up * 0.02f;
            transform.forward = -BackT.right;
            transform.eulerAngles = new Vector3(transform.eulerAngles.x - 8.5f, transform.eulerAngles.y - 3.0f, transform.eulerAngles.z);
        }
        else
        {
    ...
    IEnumerator ReloadAni()
    {
        ...
        while (time <= 0.9f)
        {
            ...
            transform.position = Pront + transform.right * 0.185f + -transform.forward * 0.20f + -transform.up * 0.06f;
            transform.forward = -BackT.right;
